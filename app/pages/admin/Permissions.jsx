      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-gray-600">Manage user access rights and module permissions.</p>
          </div>
        </div>
      </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <Icon className={`h-5 w-5 ${moduleColors[module]}`} />
                      <div>
                        <h4 className="font-medium text-gray-900 capitalize">{module}</h4>
                        <p className="text-sm text-gray-500">{usersWithAccess} users have access</p>
                      </div>
                    </div>
                    <div className="text-sm text-gray-500">
                      {Math.round((usersWithAccess / (users?.length || 1)) * 100)}% coverage
                    </div>
                  </div>
                );
              })}
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Badge className="bg-red-100 text-red-800">{adminUsersCount}</Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-600">Manager Users</span>
                <Badge className="bg-blue-100 text-blue-800">{managerUsersCount}</Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-600">Regular Users</span>
                <Badge className="bg-gray-100 text-gray-800">{regularUsersCount}</Badge>
              </div>
              <hr className="my-4" />
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-600">Full Access</span>
                <span className="text-sm text-gray-900">{adminUsersCount} users</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-600">Limited Access</span>
            ...
            <Table>
              ...
              <TableBody>
                {getAllUsersMutation?.isPending ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={modules.length + 3} className="py-8 text-center">
